...
/// `load()` reads an external .nnue from here in preference to RAZOR_NET and the
/// embedded net. The net loads once (OnceLock) on first eval; UCI sets options
/// before the first `go`, so an EvalFile set at launch takes effect.
...
/// before the first evaluation (the net loads once, then is fixed). Returns false
...
/// Whether the net has already been initialized (so an EvalFile change after this
