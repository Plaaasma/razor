...
2. **gen2 datagen labeled by v0.5.0 NNUE** — **RUNNING (launched ~02:40 2026-06-13, overnight).** 28 workers, 100M target, `H:\RazorBot\data\gen2\shard-*.txt`. ~8.5k pos/s (3.5× slower than gen1's PSQT datagen: NNUE eval 2.3× + sharper/longer games = more nodes/position). ETA ~3.3 hr (~06:00). Output is white-relative (datagen fixed) → feed `bullet-utils convert --from text` directly, skip convert_gen1.py. Then interleave → gen2.bin → train net2.
