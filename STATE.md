...
**Where we are:** Razor **v0.9.0** (tag `v0.9.0`) = v0.8.0 + **singular extensions** (+35.4 STC / +16.2 LTC, rows 43/44). = v0.7.0 net (768, `razorsf.nnue`) + i32 acc + log-LMR + singular ext. bench **20249** (singular fires depth≥8 only), archive `masters\razor-v0.9.0.exe`. v0.8.0 was 5.0%/−511 vs SF18; v0.9.0 M1 recheck pending (expect ~−476, M1 −400 ~75 Elo away).

**HEAD = v0.9.0 = v0.7.0 net + i32 acc + log-LMR + singular ext. bench = 20249** (singular fires depth≥8 only, so bench unchanged from v0.8.0). Verify `target\release\razor.exe bench` → `Nodes searched : 20249`. `src\nnue.rs`: `HIDDEN=768` + `include_bytes!("../nets/razorsf.nnue")` + **i32** acc (NOT i64). `src\search.rs`: log-LMR `lmr_table()` + singular ext (per-ply `excluded[ply]`). Tagged **v0.9.0** (singular LTC-confirmed +16.2); v0.8.0 also archived.
...
**RUNNING (2026-06-15): singular LTC-confirm** — bg `but5mx2ur`, `matches\ltc_singular.ps1` (40+0.4, 300 games), `razor-singular.exe` vs v0.8.0. On hold → tag **v0.9.0** (archive `masters\razor-v0.9.0.exe`) → M1 recheck vs SF18 (expect ~−476, closer to M1 −400). 
