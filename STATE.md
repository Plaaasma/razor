...
**★★ BIGGEST UNTAPPED LEVER = LAZY SMP (2026-06-16). The official M1 benchmark (`scripts\vs_stockfish.ps1`) runs 8 THREADS, but Razor is SINGLE-THREADED — so M1 is currently a 1-thread Razor vs 8-thread SF18 (crippled). Implementing lazy SMP (Razor actually uses 8 threads) ≈ +100-150 Elo AT M1 CONDITIONS — plausibly passes M1's ≥10% alone.** This was wrongly dismissed earlier (the SPRT *ladder* is 1T, so SMP shows no ladder Elo — but M1 is 8T, where it's huge). **THIS IS THE NEXT MAJOR FEATURE.**
  - Lazy SMP impl: spawn N Searchers (each own killers/history/cont_hist/acc stack/eval_stack), SHARING the TT. Core work = make `Tt` thread-safe (lockless: atomic or unsafe-shared entries, races tolerated — standard chess TT). Aggregate: main thread's best move (or vote); stop flag shared (AtomicBool). Honor the `Threads` UCI option (already advertised, currently ignored).
  - VALIDATE: can't use the 1T ladder. Use self-play SMP(8T) vs v0.11.0(1T) same TC (measures SMP Elo) + the real M1 (8T vs SF18, vs_stockfish.ps1 or the STC-8T proxy).
