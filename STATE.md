...
**RUNNING (2026-06-15): IIR LTC-confirm → tag v0.10.0** — bg `bzz6euah8`, `matches\ltc_iir.ps1` (iirbase vs v0.9.0, 40+0.4 300g). On hold → tag **v0.10.0** (= v0.9.0 + IIR), archive `masters\razor-v0.10.0.exe` (bench 20137). 
**LMP retry FAILED −95.8 (row 48)** — far too aggressive (bench halved); LMP doesn't suit this engine without a much gentler (SPSA-tuned) limit. Abandoned. HEAD = v0.9.0+IIR (LMP never applied).
