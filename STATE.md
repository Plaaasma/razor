...
**HEAD = v0.7.0 line + i32 acc revert + log-LMR. bench = 20249** (was 22141 at v0.7.0 tag; i32 revert kept it 22141, then LMR changed the tree → 20249). Verify `target\release\razor.exe bench` → `Nodes searched : 20249`. `src\nnue.rs` must have `HIDDEN=768` + `include_bytes!("../nets/razorsf.nnue")` + **i32** accumulation (NOT i64). search.rs has the log-LMR `lmr_table()`. Not yet re-tagged (still "v0.7.x line"); tag v0.8.0 after LTC-confirming the LMR+i32 batch.

**NOTHING RUNNING. NEXT: correction history (corrhist).** Headline next feature — smooths the spiky eval (user-reported) AND gains Elo, no speed tax. Build on the current LMR base, SPRT alone vs the new base (freeze current HEAD as the base binary first). Then cheap LMR follow-ups (history-based reduction, reduce-less-in-PV, reduce-more-when-not-improving) + improving heuristic. Then LTC-confirm the batch (i32+LMR+corrhist) → tag v0.8.0 → M1 recheck vs SF18.
