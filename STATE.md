...
---
## ★★★★★★★★★★ THREAT PROGRAM CONCLUSION (2026-06-21, rows 96-106) ★★★★★★★★★★
GENUINE EVAL BREAKTHROUGH, NOT A SHIPPABLE GAIN. Threat inputs broke the dozen-deep eval-cap wall: +80 Elo EQUAL-NODES (768 i16), first Razor-trainable eval improvement ever. But it does NOT ship at real TC — exhaustively confirmed: i16 +80→0.0@8T; all-i8 +55→+1.2 (1Tc4); split-quant (piece i16+threat i8, eval-preserved +78)→−8; 1024 i8→no gain (i8 caps eval ~+55 regardless of width); SF-scale old data→worse (+33). ROOT WALL = the discovered-ray PRODUCER geometry costs ~0.5x nps (producer-bound, NOT bandwidth: i8/split-quant/producer-opts only moved nps ~2-15%). Model: ship_Elo ≈ eqnodes − producer_penalty(~70-80) ≈ 0 for every variant. SF ships its (bigger) threats only via decades of precomputed-DirtyThreat + full VNNI/NNZ SIMD — a massive rewrite beyond reasonable ROI here. **SHIPPABLE CEILING = v0.12.6** (M1 passed r69, +19.6 search this era). within-50 of SF (+280 @8T) NOT reachable. Threat code (incremental, fused-i8, split-quant, producer-opts, all gate-green + parity-tested) preserved in worktree ab1eba5 + candidates\ for any future SF-class-producer attempt. SF corpus 1.1TB on dl380. The campaign's real deliverables: proved eval IS the gap (SF-net +308 eqnodes), broke the eval-cap wall (threats +80 eqnodes), banked +19.6 search (v0.12.6), M1 milestone. HEAD v0.12.6, git-clean.
...
**★ DISK BLOCKER RESOLVED (2026-06-21) — campaign LIVE.** H: was only-checked drive (39GB). Real capacity: **local E: 187GB + G: 185GB + C: 67GB (~440GB ≈ 8B positions), train on 4070Ti reading local. dl380 (192.168.50.203) 3.1TB free / 28c / NO GPU (archive + CPU datagen). spark (192.168.50.13) GB10 GPU + 300GB / 20c (2nd trainer). mac M4 164GB no-CUDA.** CAMPAIGN (within-50 eval) infra:
- DATA SCALE: downloading test80-2023 v6 (jun,jul,sep,oct,nov,dec, ~2B) to E:\sfdata → ~3B total w/ existing 1.09B. Full 10-20B (2022/test79/test78) → dl380 if PHASE A positive.
- ARCH: threat-inputs design workflow waudnybzv (the missing eval feature) + 32 king-buckets (vs big2's 16).
- PHASE A (de-risk): 32kb + ~3B + threats on 4070Ti, eqnodes vs v0.12.6. If trends up from big2 −97 → PHASE B full 10-20B on dl380/spark.
- bullet has NO native HalfKA/threats — must add a custom SparseInputType (trainer) + Razor incremental inference (workflow designing both).
...
**★★★★★★★ EVAL IS THE GAP — "eval capped" OVERTURNED (2026-06-20, row 94) ★★★★★★★** SF-NNUE compat layer (src/sfnnue.rs, loads classic SFNNv5 nn-3c0aa92af1da, source-exact HalfKAv2_hm, color-swap symmetric, behind SF_NET env, HEAD inert when unset) → Razor-search + SF-eval beats v0.12.6 by **+308 Elo / 85.5% at EQUAL NODES**. CONCLUSION FLIP: every prior "eval capped" result (1024 −70, big-net −80 eqnodes, buckets neutral, relabel +16) was misleading — those nets had lower TRAINING LOSS but were NOT genuinely stronger evals. A truly-SOTA eval lifts Razor ~+300 at equal nodes ⇒ EVAL IS THE DOMINANT LEVER, search is NOT the wall. The differentiator is TRAINING (data scale/quality/recipe), not architecture (big-net had ~SF's arch but our weak training → −80). WITHIN-50 PATH (real, now): train a genuinely-strong net using the open-source peer recipe (Reckless/Berserk/Caissa/Obsidian/Viridithas: huge self-gen data labeled by deep search, proper schedules) in a FAST-inference arch (incremental accumulator + SIMD; SF's net itself is 14x too slow at 204k nps full-refresh — not shippable, diagnostic only, GPL). Surveying multiple engine architectures+recipes next (not cloning SF). Search/speed wins still banked (v0.12.6 = +19.6 over v0.12.4). SF compat keeps as a permanent eval-quality yardstick.

**★★★★★★ ARCHITECTURE LEVER EMPIRICALLY CLOSED (2026-06-20, user authorized arch changes + unlimited compute) — SUPERSEDED by row 94: the arch wasn't the problem, the TRAINING was ★★★★★★** Built + trained the Caissa/Berserk-class net the design optimists wanted: 1024 hidden + 16 mirrored factorised king-buckets + 8 output-buckets, ALL 4 test80 binpacks, 200 superbatches (razor_net_big.rs). Train loss 0.021 early / 0.042 end vs razorsf 0.046 — genuinely fits better. Inference written + VERIFIED CORRECT (candidates\nnue-big.rs; bit-exact color-swap symmetry, king-bucket #1-risk cleared). RESULT (row 92): STC −172 (half nps, no SIMD) AND **EQUAL-NODES −80 (speed removed) — the bigger net plays WORSE in practice despite lower loss + correct inference.** Margin-rescale for its compressed eval (+Q 1102 vs 1800) recovered only ~11 Elo. CONCLUSION (now empirically proven WITH arch freedom, not just inferred): **Razor's eval is at its ceiling; lower training loss does NOT translate to playing strength; razorsf (768/jan) is near-optimally matched to Razor's search. Within-50 of SF18 is NOT reachable via NNUE architecture/data.** Only theoretical eval route left = a teacher genuinely stronger than public-test80 labels (iterated self-play past the public teacher / SF18-high-node relabel) — months, +16-capped per relabel (row 56), self-play regresses (rows 87-90), uncertain. SHIPPED v0.12.5 (TT prefetch +7.85). Bankable remaining = search/speed ~+15-22 (prefetch done; qsearch-TT, TT-aging, accumulator-inplace, etc. queued) → realistic ceiling ~3300. Big-net GPU artifacts kept for any future stronger-teacher attempt.
